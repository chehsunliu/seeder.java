...
public class MySqlSeeder implements Seeder {
    ...
    @SneakyThrows
    public MySqlSeeder(String jdbcUrl, String username, String password, String dbName) {
            ...
            for (var item : items) {
                ...
                try (var stmt = this.connection.prepareStatement(sql)) {
                    ...
                    for (var entry : item.entrySet()) {
                        if (tableInfo.columnTypes().get(entry.getKey()).equals("JSON")) {
    ...
    public MySqlSeeder withGetFilePath(Function<String, Path> getFilePath) {
